...
GameState* OthelloGameState::creator() {return new OthelloGameState;}
Player* OthelloPlayer::creator(bool human, int player_num) {
    // FIXME hack:
    ComponentTraceBegin();
    TRACE(INFO, "Managed wrapper for Othello launched");

    return new OthelloPlayer(human, player_num);}
...
void OthelloGameState::reset()
{
    ...
    if (m_initial_position)
    {
        ...
        m_player_up = (read_pointer[1] == 'W');  // Throw away newline character
...
const Player* OthelloGameState::player_ahead() const
{
    int black = m_player_cells_history[m_move_number][eBlack];
    int white = m_player_cells_history[m_move_number][eWhite];

    return black > white ? m_players[eBlack] :
           black < white ? m_players[eWhite] : NULL;
}
...
bool OthelloGameState::valid_move(GameMove move)
{
    ...
    if (move == PASSING_MOVE)
    {
        ASSERT(m_move_number == 0 || m_move_history[m_move_number-1] != PASSING_MOVE);
...
RESULT OthelloGameState::apply_move(GameMove move)
{
    ...
    const PlayerCode opponent = (m_player_up == eWhite) ? eBlack : eWhite;
    bool valid_move = false;
    int flipped_count = 0;

    #if OTH_GAME_STATE_LIST

        const CellState (&cur_board)[OTH_DIMENSION+2][OTH_DIMENSION+2] = m_boards[m_move_number].m_cells;
        CellState (&next_board)[OTH_DIMENSION+2][OTH_DIMENSION+2] = m_boards[m_move_number+1].m_cells;
        ...
        {                                                                   \
            ...
            {                                                               \
                ...
                else  /* cur_board[tx][ty] must be m_player_up */           \
                {                                                           \
                    if (flipped)                                            \
                    {                                                       \
                        valid_move = true;                                  \
        ...
        UndoRecord& undo = m_undo_history[m_move_number];
        ...
        {                                                                   \
            ...
            if (flipped && m_board[tx][ty] == m_player_up)                  \
            {                                                               \
                valid_move = true;                                          \
                while (flipped--)                                           \
                {                                                           \
                    tx -= dx; ty -= dy;                                     \
                    m_board[tx][ty] = m_player_up;                          \
    ...
    if (valid_move)
    {
        m_move_history[m_move_number].x = x;
        m_move_history[m_move_number].y = y;
        ++m_move_number;

        #if OTH_GAME_STATE_LIST
            m_boards[m_move_number].m_cells[x][y] = m_player_up;
        ...
        m_player_cells_history[m_move_number][m_player_up] = m_player_cells_history[m_move_number-1][m_player_up] + flipped_count + 1;
        m_player_cells_history[m_move_number][opponent] = m_player_cells_history[m_move_number-1][opponent] - flipped_count;
...
RESULT OthelloGameState::apply_passing_move()
{
    if (m_cells_available == 0 ||  // Must not be at end of game
        m_move_history[m_move_number-1] == PASSING_MOVE)  // Can't pass twice in a row
    {
        return FAIL;
    }
    ...
    m_player_cells_history[m_move_number+1][eBlack] = m_player_cells_history[m_move_number][eBlack];
    m_player_cells_history[m_move_number+1][eWhite] = m_player_cells_history[m_move_number][eWhite];
    m_player_up = (m_player_up == eWhite) ? eBlack : eWhite;
    ++m_move_number;
...
void OthelloGameState::undo_last_move()
{
    ...
    {
        #if OTH_GAME_STATE_LIST
            ASSERT(m_boards[m_move_number+1].m_status == Board::eDirty);
        #else
            UndoRecord& undo = m_undo_history[m_move_number];
            m_board[undo.added_piece.x][undo.added_piece.y] = eEmpty;
            Cell* cell = undo.flipped_pieces;
            do {m_board[cell->x][cell->y] = m_player_up; ++cell;}
            while (cell->x != -1);
        #endif
...
Value OthelloGameState::position_val() const
{
    ...
    // if it can't find any legal moves to apply.
        ...
        if (m_cells_available == 0 ||
            m_player_cells_history[m_move_number][eBlack] == 0 ||
            m_player_cells_history[m_move_number][eWhite] == 0)
        {
            return m_value_history[m_move_number] = game_over_val();
        }
    #endif
    ...
    {
        value = m_player_cells_history[m_move_number][eBlack] - m_player_cells_history[m_move_number][eWhite];
    ...
    else
    {
        ...
        value = 2000 * (black_corners - white_corners) - 1000 * (black_dangers - white_dangers);  // FIXME: magic numbers
    ...
    return m_value_history[m_move_number] = value;
}
...
bool OthelloGameState::game_over()
{
    ...
    if (SUCCEEDED(apply_passing_move()))
    {
